...
server-rendered static HTML (Docusaurus-style static site generation), so
...
DOCS_SITEMAP = "https://kyverno.io/sitemap.xml"
...
def _fetch_sitemap_urls(client: httpx.Client) -> list[str]:
    resp = client.get(DOCS_SITEMAP)
...
def crawl_docs_site(limit: int | None = None) -> int:
    ...
    with httpx.Client(timeout=30, follow_redirects=True) as client:
        urls = _fetch_sitemap_urls(client)
        if limit:
            urls = urls[:limit]
        for url in urls:
            try:
            ...
            body_match = re.search(r"<main[^>]*>(.*?)</main>", resp.text, re.I | re.S)
            html_body = body_match.group(1) if body_match else resp.text
